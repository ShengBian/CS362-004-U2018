 ...
 * Author: Sheng Bian
 * Date: July 22, 2018
 * Description: This is unit test suite for card village.
 * The rule for card village is draw 1 card and gain 2 actions.
 ***********************************/
...
int main () {
    ...
    cardEffect(village, choice1, choice2, choice3, &postState, handpos, &maxBonus);

    // ----------- TEST 1: player 1 draws 1 cards from deck--------------
    printf("\nTEST 1: player 1 draws 1 cards from deck\n");
    assert(postState.deckCount[player1],prevState.deckCount[player1]-1);

    // ----------- TEST 2: player 1 cards in hand not changed --------------
    printf("\nTEST 2: player 1 cards in hand not changed\n");
    assert(postState.handCount[player1], prevState.handCount[player1]);

    // ----------- TEST 3: player 1 gains 2 actions --------------
    printf("\nTEST 3: player 1 gains 2 actions\n");
    assert(postState.numActions, prevState.numActions+2);
